...
const todayWeatherContent = function (data) {
  const {
    city: { name, sunrise, sunset },
  } = data;

  const lastWeatherReport = data.list.pop();
  const {
    main: { temp },
    wind: { deg, speed },
  } = lastWeatherReport;

  const { main, description, icon } = lastWeatherReport.weather[0];
  ...
  <div class="flex flex-col justify-center items-center">
    ...
    <p class="WeatherName text-2xl font-bold uppercase">${main}</p>
    <p class="weatherDescription text-2xl">${description}</p>
  </div>
...
export default todayWeatherContent;
